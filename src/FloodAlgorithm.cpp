...
void FloodAlgorithm(std::vector<std::vector<gridItem>>& gridItems, sf::RenderWindow& window, sf::Vector2i& source, sf::Vector2i& target, bool& escapePressed) {
	...
	while (!gridStack.empty() && !escapePressed) {
		...
		if (currentX - 1 >= 0) {
			up = getGridAt(sf::Vector2i{ currentX - 1, currentY }, gridItems);
		...
		if (currentX + 1 < verticalGridCount) {
			down = getGridAt(sf::Vector2i{ currentX + 1, currentY }, gridItems);
		...
		if (currentY - 1 >= 0) {
			left = getGridAt(sf::Vector2i{ currentX, currentY - 1 }, gridItems);
		...
		if (currentY + 1 < horizontalGridCount) {
			right = getGridAt(sf::Vector2i{ currentX, currentY + 1 }, gridItems);
